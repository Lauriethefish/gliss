...
class NaiveNativeBlockProjection(private val settings: BlockProjectionSettings) : BlockProjection {
    ...
    override fun update() {
        ...
        val fromWorld = (settings.fromCentreBlock.world as CraftWorld).handle
        val toWorld = (settings.toCentreBlock.world as CraftWorld).handle
        ...
        for(x in -settings.projectionWidth..settings.projectionWidth) {
            ...
            for(y in -settings.projectionHeight..settings.projectionHeight) {
                for(z in -settings.projectionWidth..settings.projectionWidth) {
                    ...
                    val originBlockPos = BlockPos(x + settings.fromCentreBlock.x, y + settings.fromCentreBlock.y, z + settings.fromCentreBlock.z)
                    ...
                    val renderedState: BlockState = if(intersects(playerRelativePos, relativePos)) {
                        ...
                        toWorld.getBlockState(BlockPos(relativePos.x + settings.toCentreBlock.x,
                                relativePos.y + settings.toCentreBlock.y,
                                relativePos.z + settings.toCentreBlock.z))
                    }   else {
                        // Otherwise the block should be that of the origin of the portal
                        fromWorld.getBlockState(originBlockPos)
                    }
